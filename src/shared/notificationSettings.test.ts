...
describe('notification settings', () => {
  ...
  it('moves reminders inside a same-day quiet window to its end', () => {
    ...
    expect(adjustTaskReminderForQuietHours(reminder, {quietHoursStartLocalTime: '22:00', quietHoursEndLocalTime: '23:00', snoozeDurationMinutes: 60, taskRemindersEnabled: true, recurringTaskRemindersEnabled: true})).toBe(reminder);
    expect(adjustTaskReminderForQuietHours(quietReminder, {quietHoursStartLocalTime: '22:00', quietHoursEndLocalTime: '23:00', snoozeDurationMinutes: 60, taskRemindersEnabled: true, recurringTaskRemindersEnabled: true})).toBe(new Date(2026, 6, 27, 23, 0).getTime());
  });
  ...
  it('moves overnight reminders to the correct local next end', () => {
    ...
    expect(adjustTaskReminderForQuietHours(lateReminder, {quietHoursStartLocalTime: '22:00', quietHoursEndLocalTime: '07:00', snoozeDurationMinutes: 60, taskRemindersEnabled: true, recurringTaskRemindersEnabled: true})).toBe(new Date(2026, 6, 28, 7, 0).getTime());
    expect(adjustTaskReminderForQuietHours(earlyReminder, {quietHoursStartLocalTime: '22:00', quietHoursEndLocalTime: '07:00', snoozeDurationMinutes: 60, taskRemindersEnabled: true, recurringTaskRemindersEnabled: true})).toBe(new Date(2026, 6, 28, 7, 0).getTime());
  });
});
