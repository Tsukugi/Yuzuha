...
describe('money CSV import', () => {
  it('parses current exported rows, quoted notes, and currency totals', () => {
    ...
    expect(preview.entries[0]).toMatchObject({id: 'money_import_1', amountMinor: 1250, note: 'Lunch, shared\nwith team'});
  ...
  it('reports duplicate, reference, split, and malformed rows without creating partial entries', () => {
    ...
    const row = (id: string, accountId = 'account_everyday', splitId = '') => [
      '1', '28', id, 'expense', '100', 'EUR', accountId, 'category_food', 'Food', 'note',
      '2026-07-28T12:00:00.000Z', '2026-07-28T12:00:00.000Z', '2026-07-28T12:00:00.000Z', splitId,
    ].join(',');
    const existing: MoneyEntry = {
      id: 'money_existing', kind: 'expense', amountMinor: 1, currency: 'EUR', accountId: 'account_everyday', categoryId: 'category_food', category: 'Food', note: '', occurredAt: '2026-07-28T12:00:00.000Z', createdAt: '2026-07-28T12:00:00.000Z', updatedAt: '2026-07-28T12:00:00.000Z', splitId: null,
    };
