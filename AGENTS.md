...
The task-reminder pass is implemented: an open task can have one optional Android local reminder. The reminder uses a strict local date-time, Android notification permission, a stable task ID, AlarmManager, and boot rescheduling. Completing, deleting, clearing, or replacing a reminder cancels the old schedule. Tapping the notification opens the matching task in edit mode on Android. Broader notification settings, snooze actions, and sync remain planned.
