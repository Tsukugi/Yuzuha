...
Current periodic-money status: a new operation starts from the normal Money add form. The Repeat toggle reveals cadence, interval, first local date, and Mon/Tue/Wed/Thu/Fri/Sat/Sun toggles when the cadence is every day. The separate Periodic money view is list-first and supports pause, resume, and confirmed delete; existing generated entries stay unchanged. App schema 33 stores the selected days. No migration, network request, worker, timer, or polling loop is added. Check implementation evidence before describing this as shipped.
