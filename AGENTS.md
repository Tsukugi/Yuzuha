...
Current release status: GitHub release `Tsukugi/Yuzuha` `v0.1.7` is published with the signed Android OTA bundle and metadata for native `0.1.3`; native `v0.1.3` remains the current APK baseline, and the local release keys remain outside the committed tree.
...
Current schema boundary: the periodic weekday pass supersedes the earlier schema notes. The unreleased build accepts only app schema 33 and repository schema 3; older app, SQLite, backup, and CSV versions are rejected, with no migration path.
...
Current theme status: Settings Appearance supports System, Light, and Dark contrast modes plus five temporary color palettes: Moss, Ocean, Sunset, Plum, and Citrus. The palette and contrast choices are React view state only, reset on relaunch, and do not change AppData, exports, or background behavior. Preview-only concept cards are in `docs/theme-ideas/`. Check `src/app/theme.tsx`, the Settings test, and the theme test before describing this as shipped.

The phase-by-phase paragraphs below retain old schema numbers as historical review notes. The current boundary above is authoritative: schema 33 and repository schema 3 only, with no legacy migration or import path.

Historical schema sequence: the task-reminder pass added schema 17 `reminderAtMillis` task data; later passes added notification settings, dependencies, task order, projects, focus sessions, subtasks, and templates. The schema 28/repository schema 2 wording in this historical paragraph is not the current compatibility boundary. The current unreleased build accepts only schema 33 and repository schema 3; old migration/import paths are removed. Broader notifications, account recovery, device enrollment, sync, and later full-product phases remain planned.

Historical implementation inventory: the current core inventory from the earlier Money-filtered-totals pass is kept below for context. Its repository schema 2 and schema 28 references are historical; the authoritative current boundary is schema 33/repository schema 3 above. Check implementation evidence before describing planned work as shipped.

The current core status supersedes earlier pass labels: the current Android slice includes the calendar-draft pass, latest-import-undo boundary, local note links, and weekday-aware periodic money described below. A fresh install starts directly at schema 33; older app data and backup versions are intentionally rejected until a public upgrade policy exists. Check implementation evidence before describing planned work as shipped.
...
Current device-test installer status: Android `localTest` uses package `dev.yuzuha.localtest` and the embedded current bundle only. It disables remote metadata/download for device UI smoke without clearing the production `dev.yuzuha` workspace. Production release builds keep the normal verified OTA behavior.
...
Current periodic-money status: a new operation starts from the normal Money add form. The Repeat toggle reveals cadence and interval; when the cadence is every day, it shows compact Mon/Tue/Wed/Thu/Fri/Sat/Sun toggles. New operations start today and only selected weekdays are applied. The separate Periodic money view is list-first and supports pause, resume, and confirmed delete; existing generated entries stay unchanged. App schema 33 stores the selected days. No migration, network request, worker, timer, or polling loop is added. Check implementation evidence before describing this as shipped.
