...
The current core now also includes schema 19 notification settings with the local 15/30/60/120-minute snooze policy and Android `Snooze` action. The existing fixed one-hour behavior is the default. Check implementation evidence before describing planned work as shipped.
