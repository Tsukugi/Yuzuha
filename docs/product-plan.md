...
Add optional encrypted sync, device enrollment, conflict resolution, encrypted backup, recovery, import tools, and data portability. The current Android slice already has a strict current-format money CSV append path; arbitrary bank mapping, import history, and sync restore remain future work.
