...
Current schema boundary: app schema 33 and repository schema 3 are the only accepted versions. Payees are typed JSON source records; `money_entries.payee_id` stores each nullable stable reference. `weekStartsOn` stores the current Sunday/Monday local-week preference. Money recurrence rules store selected weekdays for every-day operations. `lastMoneyCsvImport` stores only the latest money CSV receipt with its source, import time, and imported entry timestamps. `noteLinks` stores stable note-to-task/project/money/focus-session links. Global Search derives linked-target search fields in memory and stores no index. Note rich formatting is parsed from the existing plain `Note.body` string and adds no field. Older schemas are rejected without migration.
...
The current implementation uses app schema 33 and repository schema 3. The older schema numbers in the next status paragraph are retained only to explain the sequence of completed passes.
...
Money CSV import is also external file input only. It preserves current money entry IDs and timestamps, requires current account/category references, rejects split-linked rows, and appends validated rows to the existing `money` collection only after confirmation. The latest receipt is part of current app data and is exported, encrypted, and stored in SQLite metadata. Undo is allowed only while every receipt entry exists with the same `createdAt` and `updatedAt`; a missing or edited entry blocks the whole undo with no write. A second import replaces the receipt, so only the latest import is undoable. There is no legacy format or import history.
...
### Latest money CSV import receipt

| Field | Type | Rule |
| --- | --- | --- |
| `sourceName` | string | Trimmed display name, bounded to 200 characters. |
| `importedAt` | UTC datetime | Time the confirmed import was saved. |
| `entries` | array | One record per imported entry, bounded to the current CSV row limit. |
| `entries[].id` | UUID/string | Imported money entry ID. |
...
| `weekdays` | array | Unique values from Sunday `0` through Saturday `6`; for every-day operations this is the selected Mon/Tue/Wed/Thu/Fri/Sat/Sun set. Other cadences store all seven days. |
| `nextOccurrenceLocalDate` | `YYYY-MM-DD` | Next local calendar date to generate. |
| `missedOccurrencePolicy` | enum | Stored as the internal current value `all`; the form does not expose a missed-date choice. |
...
Note rich formatting is a presentation rule over the existing body string. Toolbar actions insert bounded Markdown-like markers, and the Notes card renders known markers while leaving unknown or incomplete text unchanged. No record, schema, migration, export envelope, or repository column is added.
...
| `parentTaskId` | UUID nullable | Optional parent task in the same list. Self-links and cycles are rejected. |
...
| `id` | UUID | Primary key. |
| `name` | string | Required after trim; maximum 80 characters; unique case-insensitively. |
| `title` / `details` | string | Copied into a new open task; title is required. |
| `priority` | enum | `low`, `normal`, or `high`. |
| `listId` | UUID | Required task-list link. The list must exist when the template is saved. |
| `projectId` | UUID nullable | Optional project link. The project must exist when the template is saved. |
| `isArchived` | boolean | Archived templates are kept but cannot create tasks. |
| `createdAt` / `updatedAt` | UTC datetime | Required. |

Using an active template creates a normal open task with no due date, parent, recurrence, reminder, or source-note link. Deleting a project or task list is blocked while a template references it. Deleting a template does not delete tasks already created from it.
...
Subtasks use the task's nullable `parentTaskId` link. The parent must exist, use the same task list, and not create a self-link or cycle. Nested subtasks are allowed through the same rule. Deleting a parent promotes its direct children to top-level tasks in the same save; grandchildren remain attached to their preserved child parent.
...
Rules expand during startup and rule creation. An optional rule `HH:mm` is combined with each generated occurrence's local date and copied to `reminderAtMillis`; future values are synchronized immediately and on startup/boot, while past values are not scheduled. One-off Android task reminders are stored on the task and rebuilt at startup and boot. Task parent links require a same-list existing parent and reject self-links and cycles; deleting a parent promotes its direct children. Task dependencies store a prerequisite task, a dependent task, and the `completed` condition; self-links, duplicates, and cycles are rejected, and an incomplete prerequisite blocks completion while leaving the dependent task open. Reminder notification actions use the task ID but add no persisted action state: `Complete` changes only an existing open, unblocked task to completed, and `Snooze` replaces the logical reminder using the selected duration. `notificationSettings` stores the required global and recurring-task reminder flags, an optional daily local window, and a 15/30/60/120-minute snooze duration; both quiet-hour values are null when disabled, and a reminder inside the window is scheduled at the window end while its logical task timestamp stays unchanged. When either relevant category is disabled, native reminder alarms are cleared but logical task timestamps remain. Broader notifications, background automation, dynamic or cross-feature templates, and editing one occurrence versus a series are planned.
...
2. App schema 33 and repository schema 3 are accepted. Older and unknown schemas are rejected clearly.
...
Current storage summary: `SqliteWorkspaceStore` writes repository schema 3, app schema 33, `week_starts_on`, `last_money_csv_import`, weekday-aware money recurrence records, note links, and the existing normalized/JSON record boundaries. JSON and encrypted backup restore use the same current app-data validator. The older inventory paragraph below is retained as historical pass wording.
