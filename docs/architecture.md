...
Android share capture is an entry-point adapter, not a new repository record. The `singleTask` activity accepts only `ACTION_SEND` text payloads with `text/plain` or no MIME type, removes consumed text/subject extras, and passes a bounded payload through `YuzuhaShareCapture`. `MainApp` subscribes to warm intents and reads the cold-start payload once. A deterministic key suppresses duplicate cold/warm delivery. The review screen saves through the existing note or task AppStore methods; it uses an Inbox task with normal priority and creates nothing before confirmation. Inputs over 20,000 characters are rejected before the native bridge and by the shared normalizer.
