...
Current schema boundary: the latest Android slice accepts app schema 33 and repository schema 3 only. The periodic weekday pass adds the selected money recurrence days; older schemas remain rejected without migration.

Status: Core implementation through the Android weekday-aware periodic-money UX pass with the full-product target architecture. The installer bridge, SQLite repository boundary, app shell, core screens, reports, account balances, transfers, split entries, normalized financial tables, budget projections, exports, local deletion, recurring money and task rules with selected weekdays and deterministic catch-up, reminders, projects, templates, focus sessions, tasks, notes, bounded rich-note rendering, attachments, share/capture adapters, launcher/widget/deep-link adapters, current-format imports/restores, selected App Time/Home/Review periods, Money filters/totals/reports, the persisted week-start setting, latest-import undo, note links, local linked-target search, owning-tab search navigation, and exact money, note, task, project, task-template, task-list, app-group, and budget focus from search exist. App groups and budgets also have local edit controls. The accepted latest-only data boundary is app schema 33 and repository schema 3. Selected timezone, calendar reads/event IDs, broader notification automation, dynamic shortcuts, iOS share handling, app blocking, sync, and advanced adapters remain planned.
...
Current Settings addition: Home exposes a dedicated Settings view with the Appearance disclosure and an expanded Android Code updates disclosure. Home links to Settings from its header and More home tools, while Settings is the single owner of the temporary contrast and palette choices. The Appearance control supports System, Light, and Dark contrast modes plus the Moss, Ocean, Sunset, Plum, and Citrus palettes. The update controls call the existing typed native installer bridge; JavaScript does not fetch, hash, verify, or activate executable code. Data tools reuses the same control in its collapsed secondary section. The view adds no schema, migration, permission, worker, timer, or new network path.
...
Current repeated-flow addition: Money, Notes, and Tasks use the same explicit Cancel behavior for new and edited forms, show a saving state, and disable the form action during a write. `AppStoreProvider` serializes local commits through one in-memory queue; each queued update reads the newest committed workspace before saving, so overlapping local writes cannot replace an earlier write. This changes no schema, migration, network, worker, timer, or polling behavior.

Current Money graph and theme addition: `buildMoneyChartData` expands split lines without adding parent totals, applies the current Money filter scope, keeps currencies separate, and returns bounded category and local-day series. `ThemeProvider` follows Android `useColorScheme` for System mode, supports temporary Light/Dark overrides and five temporary color palettes, and `createStyles` resolves all screen tokens from the active theme. Neither feature writes AppData, starts a worker, or adds a native chart dependency.
...
Current Tasks UX addition: `TasksScreen` derives primary All/Today/Upcoming/Completed tabs, Today/Upcoming/no-date sections, a collapsed local task overview, compact task rows, and a floating add action from existing task/list/project records. Task tools remain available below the dashboard as collapsed reminder, dependency, project, template, list, and recurring-task groups; Agenda and deeper filters remain available there too. The pass adds no schema, migration, network request, worker, timer, or polling loop.
...
Current device-test addition: the `localTest` Android build type uses a separate `dev.yuzuha.localtest` package and sets `YUZUHA_LOCAL_BUNDLE_ONLY`. `YuzuhaBundleInstaller` then selects the embedded bundle and returns `LOCAL_BUNDLE_ONLY` without checking metadata or downloading a remote bundle. This is a test-only path for validating the current JavaScript bundle on a device that already has an older production OTA cache; production release builds keep the normal verified OTA gate.
...
Current periodic-money addition: `MoneyScreen` uses the existing money-entry form for one-time and periodic creation. The Repeat choice reveals cadence and interval, and a compact selected-weekday row for every-1-Day operations. New operations use today as their internal start date; startup skips unselected weekdays and performs one deterministic local-date expansion. The store validates and saves the current `MoneyRecurrenceRule` with app schema 33 and expands due dates in the same local commit. `MoneyRecurrenceScreen` is management only: it sorts active operations before paused ones, allows pause/resume through the AppStore, and confirms deletion while leaving generated money entries untouched. No migration, network request, worker, timer, or polling loop is added.
...
Detailed schema numbers in older implementation paragraphs below are historical pass notes. The current boundary above is authoritative: app schema 33 and repository schema 3 only.
...
Home owns a derived Money widget only. It reads the current local workspace, calculates the main-currency account balance and selected-period money activity, and routes to the existing Money list when pressed. It has no persistence or native background work. Normal tabs, deep links, and launcher add actions keep their existing list-first or explicit-add behavior.
...
Android launcher shortcuts are static manifest resources with four actions: Add money, Add note, Add task, and App Time. `YuzuhaLaunchActions` normalizes only those action strings, clears the consumed activity action/extras, emits warm launches, and provides the cold-start action getter. `MainApp` maps the first three actions to a typed pending-add state and maps App Time to its existing tab, then closes any transient share preview. The launcher never receives record content.
