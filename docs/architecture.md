...
Current search-navigation addition: `globalSearchDestination` maps every supported result kind to one existing tab. `GlobalSearchScreen` renders each result as a local button, closes the overlay, and changes `MainApp` tab state. Task results additionally pass their stable ID through `globalSearchNavigation` into the existing `TasksScreen` focus path, which loads the current task into edit mode. Other kinds have no record ID route or exact-record focus; no schema field, network request, or background work is added.
