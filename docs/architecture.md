...
Status: Core implementation through the recurring-policy pass with the full-product target architecture. The installer bridge, SQLite repository boundary, app shell, core screens, reports, account balances, transfers, split entries, normalized financial tables, budget projections, exports, local deletion, recurring money rules with missed-occurrence policies, and validated JSON restore exist; sync and advanced adapters remain planned.
