...
Current search-navigation addition: `globalSearchDestination` maps every supported result kind to one existing tab. `GlobalSearchScreen` renders each result as a local button, closes the overlay, and changes `MainApp` tab state. Money, task, note, project, task-template, and task-list results additionally pass their stable IDs through `globalSearchNavigation` into the existing `MoneyScreen`, `TasksScreen`, and `NotesScreen` focus paths, which load the current record into edit mode. Other kinds have no record ID route or exact-record focus; no schema field, network request, or background work is added.
