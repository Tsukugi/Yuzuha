...
Current Money filter addition: `MoneyScreen` applies one typed derived filter to the existing non-split entry list. It supports All/Day/Week/Month, expense/income, category, and account choices, keeps an archived selected category or account visible, and adds no schema, migration, network request, or background operation.
