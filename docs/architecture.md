...
Home owns the local Quick capture menu. It presents only the existing Money, Notes, and Tasks targets, and selecting a target changes the active tab while setting a typed pending-add state so the matching form owns validation and saving. Normal tabs and deep links remain list-first. The menu has no persistence or native background work.
