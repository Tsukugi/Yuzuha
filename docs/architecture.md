...
Current periodic-money addition: `MoneyScreen` uses the existing money-entry form for one-time and periodic creation. The repeat choice is local form state; when enabled, the store validates and saves the existing `MoneyRecurrenceRule` fields and expands due dates in the same local commit. `MoneyRecurrenceScreen` is management only: it sorts active operations before paused ones, allows pause/resume through the AppStore, and confirms deletion while leaving generated money entries untouched. Startup still performs one deterministic local-date expansion; no schema, migration, network request, worker, timer, or polling loop is added.
