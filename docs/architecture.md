...
Status: Core implementation through the encrypted-backup file-portability pass with the full-product target architecture. The installer bridge, SQLite repository boundary, app shell, core screens, reports, account balances, transfers, split entries, normalized financial tables, budget projections, exports, local deletion, recurring money rules with missed-occurrence policies, validated JSON restore, password-encrypted local backups, and encrypted backup file save/open exist; sync and advanced adapters remain planned.
