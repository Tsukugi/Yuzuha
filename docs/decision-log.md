...
## DEC-087: Focus task-template results from global search

- Status: Accepted.
- Context: The Tasks screen already owns a task-template edit form and `editingTemplateId` state. A task-template result can use that form without adding another route or changing template storage.
...
- Decision: Keep one Add money entry form. A Repeat toggle is visible only for new entries; when enabled it reveals cadence and interval plus one compact row of seven weekday toggles for every 1 Day. New operations use today as their internal start date. Store app schema 33 with selected weekdays. Keep a separate Periodic money view for current-operation management, pause/resume, and confirmed delete. Keep the missed-date behavior fixed internally to `all`.
