...
Status: Traceability baseline through the 2026-07-28 Money graphs and theme pass.
...
Current UI simplification evidence: `UX-01` maps to `Disclosure`, `HomeScreen`, `GlobalSearchScreen`, `DataToolsScreen`, `MoneyScreen`, `MoneyReportScreen`, `NotesScreen`, `TasksScreen`, `AppTimeScreen`, and `FocusSessionPanel`. The 2026-07-28 signed Xiaomi smoke checks the title-free shell, compact Home Money widget, Money balance/activity card, current entry list, Add money entry, spending overview, daily movement scope, collapsed secondary sections, and the form-open state in system light and dark modes. Pure chart-data tests cover category totals, complete daily ranges, and split-line handling. The pass adds no schema, migration, network request, worker, or background process.
