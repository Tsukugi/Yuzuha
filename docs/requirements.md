...
Current search-navigation extension: Every supported Global Search result is a tappable local row. Money/account/category/transfer/split/budget/recurrence results open Money; note/saved-search results open Notes; project/list/template results open Tasks; and app-group/focus/time-goal/usage results open App Time. A money result carries its stable ID into the existing Money focus path, a task result carries its stable ID into the existing Tasks focus path, and a note result carries its stable ID into the existing Notes focus path. Each closes Search, opens its owning tab, and loads the record in edit mode. Other kinds do not promise exact-record editing, and no result mutates data.
