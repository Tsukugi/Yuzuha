...
Current search-navigation extension: Every supported Global Search result is a tappable local row. Money/account/category/transfer/split/budget/recurrence results open Money; note/saved-search results open Notes; project/task/list/template results open Tasks; and app-group/focus/time-goal/usage results open App Time. The action closes Search and changes only the selected tab; it does not mutate data or promise exact-record editing.
