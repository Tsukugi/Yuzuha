...
Current Settings extension: Home opens a dedicated Settings view. Appearance is a closed local presentation disclosure, while Android Code updates is expanded by default and uses the existing typed native installer bridge to check or prepare a verified bundle. The prepared bundle applies only after a later launch; the view never claims that a bundle is installed. Data tools keeps the same control collapsed. This adds no schema, migration, permission, worker, timer, or JavaScript download path.
