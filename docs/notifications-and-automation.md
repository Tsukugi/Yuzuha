...
Status: Android task reminders, recurring-task reminder times, local daily quiet hours, the local Task reminders category pause, and `Open`/`Complete`/`Snooze` reminder actions with a local duration setting are implemented; broader notification and automation capability remains planned.
...
The full-product target adds explicit timezone and synced notification state. The current local Task reminders category flag, quiet-hours pair, recurring-rule reminder time, and local snooze-duration policy are in app schema 21; broader category automation and timezone contracts remain planned.
