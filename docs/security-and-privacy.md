...
- Global search runs over loaded local records and current linked-target fields in memory. It does not read attachment bytes, require a new permission, send query text to a service, or write query text to telemetry. Archived project target text and deleted target content stay outside default search values. App-time results obey the existing Usage Access and included-snapshot boundary.
...
- Task reminders use generic locked-screen text. Android content and action intents carry only the opaque local task ID plus a fixed action name; the app resolves the task locally. `Complete` and configurable `Snooze` perform no remote operation and do not include task text in the notification payload. Pausing the category only changes local alarm projection and does not transmit task data.
