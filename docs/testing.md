...
Status: Current test strategy through the Android money-search-focus pass. Unit tests and Android smoke checks exist for the current implementation; the later full-product matrix remains planned. Older phase evidence below is historical and does not describe current compatibility behavior.
...
- Money-, task-, and note-focus tests verify money, task, and note results carry stable focus IDs. Emulator smoke creates a temporary EUR 12.34 entry, searches its stored amount, taps `Open Money Expense · Food`, and shows `Editing an entry`, amount `12.34`, and `Update entry`; the note and task paths remain covered by their existing smoke evidence.
