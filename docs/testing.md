...
- The signed release APK was rebuilt with Java 17 and two Gradle workers, installed on Xiaomi `42adce68` (`M2012K11AG`), and launched successfully. The current release candidate reports package `dev.yuzuha`, version `0.1.2`, and the private release signer.
...
- Money-, task-, note-, project-, template-, list-, app-group-, and budget-focus tests verify all current exact-search IDs. The app-group and budget lifecycle unit tests verify update identity/archive behavior. Emulator smoke creates a temporary `SearchGroup`, searches it, taps `Open App group SearchGroup`, and shows `Edit app group`, name `SearchGroup`, package `com.example.searchapp`, and `Update app group`; deleting the focused group returns to `App group name` and `Add app group`. The budget smoke creates a temporary EUR 12.34 Food budget, searches `Food`, taps `Open Budget Budget · Food`, shows `Edit budget`, `Budget limit` `12.34`, and `Update budget`, then deletes the temporary budget. The money, note, task, project, template, and list paths remain covered by their existing smoke evidence.
