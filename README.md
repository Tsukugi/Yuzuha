...
The current Android reminder notification also supports a fixed `Snooze 1h` action. It replaces the logical reminder with one hour after the action time and reschedules through local quiet hours; user-selected durations, recurring notifications, and sync remain planned.
